...
class MLPBaseline(nn.Module):
    """Two-layer per-pixel MLP using 1x1 convolutions."""
    def __init__(self, n_input_channels: int, n_output_channels: int, hidden_dim: int = 128, dropout: float = 0.0):
        super().__init__()
...
def main():
    ...
    mlp_params = config.get("model_mlp", {})
    model = MLPBaseline(n_inputs, n_outputs, hidden_dim=mlp_params.get("hidden_dim", 128), dropout=mlp_params.get("dropout", 0.0))
    ...
    metrics_logger = MetricsLogger(save_dir=os.path.join("results", "mlp_baseline"))
