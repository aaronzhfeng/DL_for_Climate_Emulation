...
def main():
    ...
    cnn_params = config.get("model_cnn", {})
    model = SimpleCNN(n_inputs, n_outputs, init_dim=cnn_params.get("init_dim", 64), depth=cnn_params.get("depth", 4), dropout_rate=cnn_params.get("dropout_rate", 0.2))
    ...
    metrics_logger = MetricsLogger(save_dir=os.path.join("results", "cnn_baseline"))
